...
GPIO.setup(dark_pin, GPIO.OUT)
GPIO.output(dark_pin, False)
...
def detect_and_draw(frame):
    ...
    width_half = int(gray.shape[1]/2)
    height_half = int(gray.shape[0]/2)
    dim = (width_half, height_half)
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.rectangle(frame, (2*x, 2*y), (2*(x+w), 2*(y+h)), (0, 0, 255), 2)
        ...
        for (x2, y2, w2, h2) in smile:
            ...
            cv2.rectangle(roi_color, (2*x2, 2*y2), (2*(x2+w2), 2*(y2+h2)), (0, 255, 0), 2)
            cv2.putText(frame,'Smile',(2*x,2*(y-7)), 3, 1.2, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.imwrite("/media/pi/ESD-USB/"+"photo/"+now+".jpg", save_frame)
